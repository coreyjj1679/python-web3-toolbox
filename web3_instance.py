...
class Web3Instance:
    ...
    def get_latest_block(self) -> BlockInfo:
        return {"block_height": self.provider.eth.get_block('latest')['number'],
    ...
    def get_latest_block_custom(self, custom_chain):
        ...
        custom_network = next(item for item in networks if item["chain_slug"] == custom_chain)

        if custom_network:
            temp_provider = self.connect_web3(custom_network['rpc'])
            return {"block_height": temp_provider.eth.get_block('latest')['number'],
                    "timestamp": temp_provider.eth.get_block('latest')['timestamp']}

    def get_balance(self, address, block=None):
        return str(Web3.from_wei(self.provider.
                                 eth.get_balance(address, block_identifier=int(block) if block else 'latest'), 'ether'))
