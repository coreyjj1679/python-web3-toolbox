...
class UserConfig:
    ...
    def __init__(self):
        ...
        self.wallet_list = [
            WalletConfig(i["address"], i["name"])
            for i in json.loads(open(wallet_path, "r").read())
        ]
        self.bookmark_list = json.loads(open(bookmark_path, "r").read())
        self.cur_network = next(
            (item for item in self.network_list if item["chain_slug"] == self.chain),
            None,
        )
