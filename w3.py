...
def block(
    chain: Annotated[
        str, typer.Option("--chain", "-c", help='custom chain slug. e.g. "bnb"')
    ] = None
) -> None:
    if chain:
    ...
    print(
        f"latest block: {block_info['block_height']}, chain_id: {chain or config.cur_network['chain_slug']}, "
...
def balance(
    block_height: Annotated[
        int,
        typer.Option(
            "--block", "-b", help='custom_block_height chain slug. e.g. "bnb"'
        ),
    ] = None,
    addresses: Annotated[
        List[str], typer.Option("--address", "-a", help="custom addresses")
    ] = None,
):
    if addresses:
        table = Table(
            "index", "address", f"{config.cur_network['native_currency']} balance"
        )
        for i, j in enumerate(addresses):
            ...
            table.add_row(
                str(i),
                cs_address,
                config.web3_instance.get_balance(cs_address, block_height),
            )
        ...
        table = Table(
            ...
            "name",
            "address",
            f"{config.cur_network['native_currency']} balance",
        )
        for i, wallet in enumerate(config.wallet_list):
            table.add_row(
                ...
                wallet.name,
                wallet.address,
                config.web3_instance.get_balance(wallet.address, block_height),
            )
    console.print(table)
...
def abi(
    address: Annotated[
        str, typer.Option("--address", "-a", help="contract address")
    ] = None,
    chain: Annotated[
        str, typer.Option("--chain", "-c", help="custom chain_slug")
    ] = None,
    output_dir: Annotated[
        str, typer.Option("--output", "-o", help="destination of the abi")
    ] = "abis",
    fname: Annotated[
        str, typer.Option("--name", "-n", help="file name of the json")
    ] = None,
):
    _abi = config.cur_network["api"]
    if chain:
        custom_chain = next(
            (item for item in config.network_list if item["chain_slug"] == chain), None
        )
        if not custom_chain:
            print(f"{chain} not found inside networks.json")
            print(
                f'supported networks: {[i["chain_slug"] for i in config.network_list]}'
            )
            return
        else:
    ...
    dest = os.path.join(
        base_path, f'{output_dir}/{chain or config.cur_network["chain_slug"]}'
    )
